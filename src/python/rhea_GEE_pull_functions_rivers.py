...
def clipImage(image):
  return image.clip(this_line.geometry())
...
def masking(image):
  ...
  # road = ee.FeatureCollection("TIGER/2016/Roads").filterBounds(this_line.geometry()).geom().buffer(30) 
  ...
  cScore = cScore.reduceRegion(ee.Reducer.sum(),this_line.geometry(), 30).get('Cloud')
