...
def clipImage(image):
  return image.clip(rivers.geometry())
...
def masking(image):
  ...
  road = ee.FeatureCollection("TIGER/2016/Roads").filterBounds(rivers.geometry()).geom
  ().buffer(30) 
  ...
  cScore = cScore.reduceRegion(ee.Reducer.sum(),rivers.geometry(), 30).get('Cloud')
